...
// Like strlen, but don't read more than max_len c8's from maybe_null_terminated
usize strnlen(c8 *maybe_null_terminated, usize max_len) {
	usize len = 0;
	while (len < max_len) {
		if (maybe_null_terminated[len] == 0) {
			return len;
		}
		len++;
	}
...
union shortstr shortstr_from_null_terminated(const c8 *null_terminated)
{
	...
	strncpy(new_shortstr.s, null_terminated, sizeof(new_shortstr.s));
	usize len = strnlen(new_shortstr.data, sizeof(new_shortstr.data)); // Just this once, data is null terminated
