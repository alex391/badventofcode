...
class Part():
    ...
    def __str__(self) -> str:
        return f"({self.part_number}, ({self.coordinate[0]}, {self.coordinate[1]}))"
        ...
        for dx in range(-1, 2):
            try:
                ...
                if engine[y + dy][x + dx].isdecimal():
                    numbers_around_symbol.append(parse_number_in_engine(engine, x + dx, y + dy))
            except IndexError:
                continue
...
def parse_number_in_engine(engine: list, x: int, y: int) -> Part:
    """
        Read an number from the engine at the coordinates (backwards or forwards)
...
def number_length(n: int) -> int:
    return int(math.log10(n) + 1)  # https://math.stackexchange.com/a/1771462 thanks https://math.stackexchange.com/users/73561/antoine
