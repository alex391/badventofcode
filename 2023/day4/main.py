...
class LinkedList():
    ...
    def insert_after(self, previous: Node, data) -> Node:
        """Take in the node to insert after, the data to insert, and return the new node"""
...
def main():
    ...
    while True:
        ...
        temp = current_card
        if current_card.next != None:
            for i in range(score):
                i += 1
                current_card = cards.insert_after(current_card, card_list[temp.data.card_number + i])
        current_card = temp
        if current_card.next != None:
