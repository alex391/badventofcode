...
class AlmanacMap():
    def __init__(self, to_parse: str):
        self.destination, self.source, self.length = [
            int(x) for x in to_parse.split()
        ]
...
def main():
    with open("input.txt") as file:
        ...
        # Take seeds (iterable containing [1, 2, 3, 4]) and convert to ranges
        # ([range(1, 3), range(3, 7)])
        # Thanks https://stackoverflow.com/a/1625023 by
        # https://stackoverflow.com/users/97828/nadia-alramli
        seed_ranges = list(
            map(lambda x: range(x[0], x[0] + x[1]), zip(*(seeds,) * 2)))
        current_map = []
        for line in file:
            line = line.strip()
            if line == "":
    ...
    # CPU go brrr
    # Thanks https://stackoverflow.com/a/28463266 by
    # https://stackoverflow.com/users/2327328/philshem
    pool = ThreadPool(len(seed_ranges))
    lowest_location_numbers = pool.map(
        lowest_location_in_seed_range, seed_ranges)
...
def lowest_location_in_seed_range(seed_range: range) -> int:
    ...
    for seed in seed_range:
        ...
        lowest_location_number = (
            seed if seed < lowest_location_number else lowest_location_number
        )
